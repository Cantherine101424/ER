...
class InterHand26M(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.joint_set = {
            ...
            'joints_name': ('R_Thumb_4', 'R_Thumb_3', 'R_Thumb_2', 'R_Thumb_1',
                            ...
                            'R_Ring_4', 'R_Ring_3', 'R_Ring_2', 'R_Ring_1', 'R_Pinky_4',
                            'R_Pinky_3', 'R_Pinky_2', 'R_Pinky_1', 'R_Wrist', 'L_Thumb_4',
                            'L_Thumb_3', 'L_Thumb_2', 'L_Thumb_1', 'L_Index_4', 'L_Index_3',
                            'L_Index_2', 'L_Index_1', 'L_Middle_4', 'L_Middle_3', 'L_Middle_2',
                            'L_Middle_1', 'L_Ring_4', 'L_Ring_3', 'L_Ring_2', 'L_Ring_1',
                            'L_Pinky_4', 'L_Pinky_3', 'L_Pinky_2', 'L_Pinky_1', 'L_Wrist'),
            'flip_pairs': [(i, i + 21) for i in range(21)],
            'regressor': np.load(osp.join('..', 'data', 'InterHand26M', 'J_regressor_mano_ih26m.npy'))
        }
        ...
        # if data_split=='train':
        #     self.sample_dataset = self.create_sample_dataset(self.datalist, target_sample_size=cfg.train_smple, allowed_deviation=0.05)
        # elif data_split=='test':
        #     self.sample_dataset = self.create_sample_dataset(self.datalist, target_sample_size=cfg.test_smple, allowed_deviation=0.05)
        # else:
        #     print('dataset chosen incorrect!')

    def create_sample_dataset(self, datalist, target_sample_size=5000, allowed_deviation=0.05):

        random.seed(42)


        hand_type_counter = Counter([data['hand_type'] for data in datalist])
        total_samples = len(datalist)
        hand_type_ratios = {hand_type: count / total_samples for hand_type, count in hand_type_counter.items()}


        target_hand_type_counts = {hand_type: int(target_sample_size * ratio) for hand_type, ratio in
                                   hand_type_ratios.items()}
        min_target_counts = {hand_type: int(count * (1 - allowed_deviation)) for hand_type, count in
                             target_hand_type_counts.items()}
        max_target_counts = {hand_type: int(count * (1 + allowed_deviation)) for hand_type, count in
                             target_hand_type_counts.items()}


        grouped_data = {hand_type: [] for hand_type in hand_type_ratios.keys()}
        for data in datalist:
            grouped_data[data['hand_type']].append(data)


        selected_samples = []
        for hand_type, samples in grouped_data.items():
            num_samples = min(target_hand_type_counts[hand_type], len(samples))
            selected_samples.extend(random.sample(samples, num_samples))  # 使用随机抽取但有固定种子


        while len(selected_samples) < target_sample_size:
            hand_type_to_add = \
            random.choices(list(hand_type_ratios.keys()), weights=list(hand_type_ratios.values()), k=1)[0]
            additional_sample = random.choice(grouped_data[hand_type_to_add])
            selected_samples.append(additional_sample)

       
        selected_hand_type_counter = Counter([data['hand_type'] for data in selected_samples])
        for hand_type, count in selected_hand_type_counter.items():
            assert min_target_counts[hand_type] <= count <= max_target_counts[hand_type], \
                f"{hand_type} count {count} is out of range [{min_target_counts[hand_type]}, {max_target_counts[hand_type]}]"
    ...
    def load_data(self):
        ...
        if self.data_split == 'test':
            rootnet_path = osp.join(self.annot_path, '..','rootnet',
                                    'rootnet_interhand2.6m_output_' + self.data_split + '.json')
            rootnet_result = {}
    ...
    def __getitem__(self, idx):
        ...
        if mano_param['right'] is not None:
            ...
            rmano_joint_img, rmano_joint_cam, rmano_joint_trunc, rmano_pose, rmano_shape, rmano_mesh_cam =\
                process_human_model_output(mano_param['right'], data['cam_param'], do_flip, img_shape,
                                           img2bb_trans, rot)
        ...
        if mano_param['left'] is not None:
            ...
            lmano_joint_img, lmano_joint_cam, lmano_joint_trunc, lmano_pose, lmano_shape, lmano_mesh_cam = \
        ...
        # print('inter--img.shape:::', img.shape)
        # print('inter--joint_img.shape:::', joint_img.shape)
        # print('inter--mano_joint_img.shape:::', mano_joint_img.shape)
        # print('inter--joint_cam.shape:::', joint_cam.shape)
        # print('inter--mano_joint_cam.shape:::', mano_joint_cam.shape)
        # print('inter--mano_mesh_cam.shape:::', mano_mesh_cam.shape)
        # print('inter--rel_trans.shape:::', rel_trans.shape)
        # print('inter--mano_pose.shape:::', mano_pose.shape)
        # print('inter--mano_shape.shape:::', mano_shape.shape)
        ...
        meta_info = {'do_flip': do_flip, 'bb2img_trans': bb2img_trans,
                     ...
                     'mano_param_valid': mano_param_valid, 'mano_shape_valid': mano_shape_valid,
                     'is_3D': float(True)}
