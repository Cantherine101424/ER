...
mano_layer = {'right': smplx.create(cfg.human_model_path, 'mano', is_rhand=True, use_pca=False),
                      'left': smplx.create(cfg.human_model_path, 'mano', is_rhand=False, use_pca=False)}
...
root_path = osp.join('/data/users/fr/', 'interhand-install', 'InterHand26M')
img_root_path = osp.join(root_path, 'images')
annot_root_path = osp.join(root_path, 'annotations')

split = 'test'
capture_idx = '0'
seq_name = 'ROM09_Interaction_Fingers_Touching'
cam_idx = '400268'
image_name='image24915.jpg'

save_path = osp.join('../render/')
...
img_path_list = glob(osp.join(img_root_path, split, 'Capture' + capture_idx, seq_name, 'cam' + cam_idx, image_name))
