...
import glob
image_name='image18580'
def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--test_epoch', type=str, default=75, dest='test_epoch')
    parser.add_argument('--gpu', type=str, dest='gpu_ids',default='1')
    parser.add_argument('--input', type=str, default='../render/'+image_name+'.jpg', dest='input')
...
model_path = os.path.join(cfg.model_dir, 'snapshot_woFusion_%d.pth.tar' % int(args.test_epoch))
...
save_obj(rmano_mesh*np.array([1,-1,-1]), mano.face['right'], image_name+'_right.obj')
save_obj((lmano_mesh+rel_trans)*np.array([1,-1,-1]), mano.face['left'], image_name+'_left.obj')
